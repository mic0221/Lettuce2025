...
and loading the train & test datasets for a patch-based model.
...
class LettuceSegDataset(Dataset):
    def __init__(
        ...
        MODIFIED: This class now internally handles all cropping logic.
        - If is_train=True: Performs a mixed strategy crop (positive or random).
        - If is_train=False: Returns the full-resolution image for sliding-window validation.

        The 'transform' parameter should ONLY contain post-crop augmentations
        (e.g., Flip, Rotate, Normalize, ToTensorV2).
        """
        self.transform = transform
        self.give_name = give_name
        self.is_train = is_train
        self.crop_size = crop_size
        self.positive_crop_pob = positive_crop_prob
    ...
    def _get_random_patch(self, img, mask):
        """Helper function for a simple random crop."""
        h, w = self.crop_size
        ...
        return img[y1:y1 + h, x1:x1 + w], mask[y1:y1 + h, x1:x1 + w]

    def _get_positive_patch(self, img, mask):
        """Helper function to crop around a positive (tipburn) pixel."""
        h, w = self.crop_size
        img_h, img_w = img.shape[:2]

        # 1. find where tipburn exists
        positive_pixels = np.argwhere(mask > 0)

        if len(positive_pixels) == 0:
            # 2. if there is not tipburn, randomly cropping
            return self._get_random_patch(img, mask)

        # 3. randomly pick a tipburn pixel for center
        center_y, center_x = random.choice(positive_pixels)

        # 4. make sure not to exceed boundary
        y1 = max(0, min(center_y - (h // 2), img_h - h))
        x1 = max(0, min(center_x - (w // 2), img_w - w))
    ...
    def __getitem__(self, index):
        ...
        # for cropping
        if self.is_train:
            if random.random() < self.positive_crop_prob:
                # 70% get tipburn area
                img_patch, mask_patch = self._get_positive_patch(img, mask)
            else:
                # 30% random area
                img_patch, mask_patch = self._get_random_patch(img, mask)
        else:
            # for validation: use whole img
            img_patch, mask_patch = img, mask

        # Apply data augmentation transforms to image and mask
        if self.transform is not None:
            augmentations = self.transform(image=img_patch, mask=mask_patch)
        ...
        if self.give_name:
            img_name = os.path.basename(os.path.normpath(self.img_names[index]))
...
def get_loaders(
    ...
    Args:
        ...
        batch_size (int): Number of samples in each batch. batch_size = 1 for testing.
