#!/usr/bin/env python3
"""
Perform inference for tipburn masks via tipburn segmentation model
It was used for a patch-based model, which used sliding window method for inference
...
def inference(
    ...
    Args:
        ...
        patch_size (tuple, optional): The patch size the model expects (e.g., (480, 480)).
        stride (tuple, optional): The stride for the sliding window (e.g., (240, 240)).

    Returns:
        tensor/tuple(tensor, list): Tensor of predictions and optionally performance as list of floats.
            Alternatively performance is a list of floats when using multiple performance metrics.
    """
    # Make predictions
    model = model.eval()
    ...
    # Check if sliding window is necessary
    if H > patch_h or W > patch_w:
        # 1. Infer output channels by running a dummy patch
        try:
            # Try to get it from the model if it's defined
        ...
        # 2. Create accumulators for logits and normalization
        ...
        # 3. Generate coordinates for all patches, ensuring edges are covered
        ...
        # 4. Normalize the accumulated logits where patches overlapped
        # Add epsilon to prevent division by zero
        logit_preds = accum_logit_preds / (normalization_map + 1e-8)

    else:
        # --- Original Logic: If no sliding window is needed ---
        with torch.inference_mode():
            logit_preds = model(data)

    # Apply sigmoid to the (potentially reconstructed) full-size logits
...
def main():
    ...
    loader = DataLoader(
        dataset,
        batch_size=1, # 8 -> 1 to deal with higher resolution
        num_workers=0,
        pin_memory=True,
        shuffle=False,
    )
